...
#!/bin/bash 를 맨앞에 해줘야 이게 스크립트 파일이다! 라고 정해주는것!
```java
#!/bin/bash
...
ls pwd whoami 가 순차적으로 실행됨!
( 스크립트실행 부분 영상 다시보면 짱좋음 echo $PATH 원리 ! )


만약 권한이 없어서 실행이 안된다면 chmod 744 파일명 등등 하면됨. chmod a+x  

근데 만약 pwd 가 너무 길어! 경로가 ! 그럼 걍 ./파일명 으로해도 된다!
